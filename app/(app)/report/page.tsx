...
export default async function ReportPage({
  ...
  const label = formatPeriodTH(period, cycleStartDay);

  if (!rows?.length) {
    return (
      <main className="mx-auto flex w-full max-w-3xl flex-col gap-4 p-4 pb-16 sm:p-6">
        <h1 className="px-1 text-2xl font-semibold">รายงาน {label}</h1>
        <GlassCard className="p-0">
          <EmptyState title="รอบนี้ยังไม่มีรายการ ไม่มีอะไรให้สรุป" />
        </GlassCard>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-col gap-4 p-4 pb-16 sm:p-6">
      <div className="no-print flex items-center justify-between gap-3 px-1">
        <div>
          <h1 className="text-2xl font-semibold">รายงาน {label}</h1>
          <p className="text-text-muted mt-1 text-sm">
            กดพิมพ์แล้วเลือก &ldquo;บันทึกเป็น PDF&rdquo; ในหน้าต่างที่ขึ้นมา
          </p>
        </div>
        <PrintButton />
      </div>

      <nav className="no-print flex gap-2 px-1" aria-label="เลือกรอบ">
        <a href={`/report?period=${shiftPeriod(period, -1)}`} className="text-text-muted rounded-full px-3 py-1.5 text-sm hover:bg-hover">
          ← {formatPeriodTH(shiftPeriod(period, -1), cycleStartDay)}
        </a>
        <a href={`/report?period=${shiftPeriod(period, 1)}`} className="text-text-muted rounded-full px-3 py-1.5 text-sm hover:bg-hover">
          {formatPeriodTH(shiftPeriod(period, 1), cycleStartDay)} →
        </a>
      ...
      <header className="print-only mb-4">
        <h1 className="text-2xl font-semibold">รายงานรายรับ-รายจ่าย</h1>
        <p className="text-text-muted mt-1 text-[15px]">
          {label}
          {displayName && ` · ${displayName}`} · ออกเมื่อ {formatDateTH(todayISO())}
        </p>
      </header>

      <GlassCard className="p-5">
        <h2 className="text-xl font-semibold">สรุปรอบ</h2>
        <table className="mt-3 w-full text-[15px]">
          <tbody>
            {[
              ["รายรับ", income],
              ["รายจ่าย", expense],
              ["คงเหลือรอบนี้", income - expense],
              ["ยอดสะสมยกมา", Number(summary?.balance ?? 0)],
            ].map(([k, v]) => (
        ...
        <p className="mt-4 text-[15px]">
          {over.length === 0 && warn.length === 0
            ? "รอบนี้ทุกหมวดอยู่ในงบ"
            : `รอบนี้ใช้เกินงบ ${over.length} หมวด และใกล้เต็มวงเงินอีก ${warn.length} หมวด`}
          {!f.isPast && f.daysGone > 0 && (
            <>
              {" "}ผ่านมา {f.daysGone} จาก {f.daysTotal} วัน ถ้าใช้อัตราเดิมต่อไป
              ปลายรอบจะเหลือราว{" "}
        ...
        {categories.length > 0 && (
          <p className="text-text-muted mt-2 text-[15px]">
            หมวดที่ใช้มากที่สุดคือ {categories[0].name}{" "}
            <span className="tabular">{formatMoney(categories[0].spent, currency)}</span>{" "}
            คิดเป็น {Math.round((categories[0].spent / Math.max(expense, 1)) * 100)}% ของรายจ่ายทั้งรอบ
          </p>
        )}
      </GlassCard>
      ...
      {(usage?.length ?? 0) > 0 && (
        <GlassCard className="p-5">
          <h2 className="text-xl font-semibold">งบแต่ละหมวด</h2>
          <table className="mt-3 w-full text-[15px]">
            <thead>
              <tr className="text-text-muted border-glass-border border-b text-left text-sm">
                <th className="py-2 font-medium">หมวด</th>
                <th className="py-2 text-right font-medium">งบ</th>
                <th className="py-2 text-right font-medium">ใช้ไป</th>
                <th className="py-2 text-right font-medium">คิดเป็น</th>
              </tr>
            </thead>
            <tbody>
              {usage!.map((u) => (
                <tr key={u.category_name} className="border-glass-border border-b last:border-0">
                  <td className="py-2">
                    ...
                    {u.status === "over" && " (เกินงบ)"}
                    {u.status === "warn" && " (ใกล้เต็ม)"}
                  </td>
                  <td className="tabular py-2 text-right font-mono">
      ...
      <GlassCard className="p-5">
        <h2 className="text-xl font-semibold">รายการทั้งหมด ({rows.length})</h2>
        <table className="mt-3 w-full text-[15px]">
          <thead>
            <tr className="text-text-muted border-glass-border border-b text-left text-sm">
          ...
          <tbody>
            {rows.map((t, i) => (
              <tr key={i} className="border-glass-border border-b last:border-0">
                <td className="tabular py-2 whitespace-nowrap">{formatDateTH(t.occurred_on)}</td>
                <td className="py-2">
